...
public class IO {
	...
	/**
	 * Lee una lista de valores separados por comas
	 * 
	 * @return Lista de valores como una lista de cadenas
	 */
	static public List<String> readList() {
	    while (true) {
	        String input = sc.nextLine();
	        if (!input.isEmpty()) {
	            // Dividir la entrada por comas y convertirla en una lista
	            return Arrays.asList(input.split("\\s*,\\s*"));
	        }
	        System.err.print("ERROR: Esta vacía la lista? ");
	    }
	}
	...
	static public Date readDate() {
        while (true) {
            try {
                ...
                System.err.print("ERROR: No es una fecha válida (yyyy-MM-dd) ? ");
            }
        }
    }
